...
#### RSSwapTool
To swap rolling stock with **RSSwapTool**, run main.py from the command prompt or just double click main.py. The main screen should then appear as a blue box with tick boxes for stock to be swapped. In Train Simulator, scenarios are stored inside the RailWorks folder where TS is installed under a folder named "Content". Content contains a folder called Routes, and in turn the Routes folder contains one folder per route and the individual route folders each contain a "Scenarios" folder where the scenario folders are kept. Each scenario folder will contain a few files, the file named "Scenario.bin" is the file that will be operated on by RSSwapTool (and RSReportTool). 
